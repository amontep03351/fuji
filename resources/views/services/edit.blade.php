                    ...
                    <form method="POST" action="{{ route('services.update', $service->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <input type="text" name="name_en" id="name_en" value="{{ old('name_en', $service->name_en) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                @error('name_en')
                            ...
                            <div>
                                ...
                                <input type="text" name="name_jp" id="name_jp" value="{{ old('name_jp', $service->name_jp) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                @error('name_jp')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <textarea name="description_en" id="description_en" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>{{ old('description_en', $service->description_en) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="description_jp" id="description_jp" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>{{ old('description_jp', $service->description_jp) }}</textarea>
                        ...
                        <div class="mt-6">
                            ...
                            <input type="number" name="display_order" id="display_order" value="{{ old('display_order', $service->display_order) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                            @error('display_order')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 mt-6">  
                            ...
                            <form id="image-upload-form" method="POST" action="{{ route('services.update.image', $service->id) }}" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <input type="file" id="edit-image-input" name="service_image" class="hidden" accept="image/*" onchange="document.getElementById('image-upload-form').submit();">
                            </form>
                        </div>
                        <hr class="my-6 border-gray-300 white:border-gray-700">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 mt-6">  
                            ...
                            @foreach($images as $image)
                                <div class="relative group overflow-hidden rounded-lg shadow-lg border border-gray-300 hover:shadow-xl transition-shadow duration-300">
                                    ...
                                    <!-- Delete Form -->
                                    <form id="delete-form-{{ $image->id }}" action="{{ route('service_images.destroy', $image->id) }}" method="POST" style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
                            ...
                            <div class="relative group overflow-hidden rounded-lg shadow-lg border border-dashed border-gray-300 hover:shadow-xl transition-shadow duration-300 cursor-pointer">
                                ...
                                <form action="{{ route('service_image.store', $service->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
