            ...
            <div class="bg-white white:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 white:text-gray-100">
                    ...
                    <div class="flex justify-between mb-6">
                        ...
                        <a href="{{ route('services.create') }}" class="inline-flex items-center px-4 py-2 bg-green-600 text-white font-semibold rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                            Add New
                        </a>
                    </div>
                        ...
                        <table id="sortable-table" class="min-w-full divide-y divide-gray-200 white:divide-gray-700">
                            <thead class="bg-gray-50 white:bg-gray-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Relate Management</th> <!-- คอลัมน์ใหม่ -->
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200 white:bg-gray-800">
                                @forelse ($services as $service)
                                    <tr data-id="{{ $service->id }}" class="sortable-row hover:bg-gray-100 white:hover:bg-gray-700 transition-colors duration-200">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 white:text-gray-100">{{ $service->display_order }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 white:text-gray-400">{{ $service->name_en }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 white:text-gray-400">{{ $service->name_jp }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('services.related.index', $service->id) }}" class="text-blue-600 hover:text-blue-900">Manage</a> <!-- ปุ่มจัดการ -->
                                        </td>     
                    ...
                    <!-- Pagination --> 
                    <div class="mt-6">
                        {{ $services->appends(['rows_per_page' => request('rows_per_page')])->links() }}
                    </div>
                </div>
            </div>
        </div>
