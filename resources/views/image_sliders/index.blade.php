            ...
            <div class="bg-white white:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 white:text-gray-100">
                    ...
                    <div class="flex justify-between mb-6">
                        ...
                        <a href="{{ route('image_sliders.create') }}" class="inline-flex items-center px-4 py-2 bg-green-600 text-white font-semibold rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                            Add New
                        </a>
                    </div>
                        ...
                        <table id="sortable-table" class="min-w-full divide-y divide-gray-200 white:divide-gray-700">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Description (EN)</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Description (JP)</th> 
                                ...
                                @forelse ($imageSliders as $imageSlider) 
                                    <tr data-id="{{ $imageSlider->id }}" class="sortable-row hover:bg-gray-100 white:hover:bg-gray-700 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-4">
                                            <a href="{{ route('image_sliders.edit', $imageSlider->id) }}" class="text-indigo-600 hover:text-indigo-900 ml-2">Edit</a>
                    ...
                    <!-- Pagination --> 
                    <div class="mt-6">
                    {{ $imageSliders->appends(request()->except('page'))->links() }}
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            $('.status-checkbox').on('change', function() {
                ...
                fetch('{{ route('image_sliders.toggle-status') }}', {
                    method: 'POST',
                    headers: {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            var sortable = Sortable.create(document.getElementById('sortable-table').querySelector('tbody'), {
                onEnd: function(evt) {
                    ...
                    fetch('{{ route('image_sliders.update-order') }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
