                ...
                <div class="p-6 text-gray-900 white:text-gray-100">
                    <form method="POST" action="{{ route('product-categories.update', $productCategory) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name_jp" id="name_jp" value="{{ old('name_jp', $productCategory->translations->where('locale', 'jp')->first()->name ?? '') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                            @error('name_jp')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
