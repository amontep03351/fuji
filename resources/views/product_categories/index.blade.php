        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6 p-6 flex items-center space-x-4">
                ...
                @php
                    ...
                    $iconClass = $currentSortOrder === 'asc' ? 'fa-arrow-up' : 'fa-arrow-down';
                    $buttonColor = $currentSortOrder === 'asc' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-red-600 hover:bg-red-700';
                @endphp
                <a
                    href="{{ route('product-categories.index', array_merge(request()->query(), ['sort_order' => $newSortOrder])) }}"
                    class="px-4 py-2 {{ $buttonColor }} text-white font-semibold rounded-md shadow-sm flex items-center space-x-2"
                >
                    <i class="fas {{ $iconClass }}"></i>
                    <span>Sort {{ $newSortOrder === 'asc' ? 'Ascending' : 'Descending' }}</span>
                </a>
            </div>
